...
if options['m'] != '' and options['p'] != '' and options['n'] != '':
    ...
    force_constants_path = os.path.join(
            os.path.split(material_input)[0], 'FORCE_CONSTANTS'
            )
    ...
    for job in range(len(job_list_recv)):
        print('Progress:', job, len(job_list_recv), 'time', (time_module.time()-start_time)/60.)
